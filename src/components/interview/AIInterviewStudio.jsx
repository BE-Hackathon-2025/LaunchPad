...
import { Mic, Video, VideoOff, MicOff, MessageSquare, Send, Sparkles, CheckCircle2, XCircle, Clock, Brain, Code, AlertCircle, Play, Pause, RotateCcw, TrendingUp, Award, Target, Zap, Eye, EyeOff, ThumbsUp, Users, Rocket, ChevronRight, ArrowRight, Volume2, VolumeX, RefreshCw, User } from 'lucide-react';
...
const AIInterviewStudio = () => {
  const [stage, setStage] = useState('setup');
  const [config, setConfig] = useState({});
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
  const [answers, setAnswers] = useState([]);
  const [currentAnswer, setCurrentAnswer] = useState('');
  const [interviewerMood, setInterviewerMood] = useState('neutral');
  ...
  const speak = async (text, retryCount = 0, onComplete) => {
    if (!voiceEnabled) {
      onComplete?.();
      return;
    }
  ...
  const gradeAnswerWithAI = async (question, answer) => {
    ...
    const hasExplanation = sentences >= 2;

    let explanationScore = 6;
    ...
    const technicalIndicators = ['implement', 'architecture', 'design', 'optimize', 'performance',
      'scale', 'algorithm', 'complexity', 'pattern', 'best practice', 'framework', 'library', 'api',
      'database', 'cache', 'async', 'sync'];
    ...
    const problemSolvingKeywords = ['because', 'therefore', 'thus', 'so', 'since',
      'due to', 'as a result', 'this allows', 'this ensures'];
    const problemSolvingMatches = problemSolvingKeywords.filter(keyword =>
      answer.toLowerCase().includes(keyword));
    const hasReasoning = problemSolvingMatches.length > 0;

    let reasoningScore = 8;
    ...
    if (finalScore >= 95) {
      feedback = `Exceptional answer (${finalScore}/100)! You demonstrated mastery with comprehensive technical details.`;
    } else if (finalScore >= 85) {
      feedback = `Excellent response (${finalScore}/100)! You covered the key concepts well with good technical depth.`;
    } else if (finalScore >= 75) {
      feedback = `Good answer (${finalScore}/100). You hit the main points but there's room to strengthen your response.`;
    } else if (finalScore >= 65) {
      feedback = `Adequate attempt (${finalScore}/100). You touched on some concepts but need more depth.`;
    } else if (finalScore >= 50) {
      feedback = `Below expectations (${finalScore}/100). Your answer lacks sufficient detail and technical accuracy.`;
    } else {
      feedback = `Needs significant improvement (${finalScore}/100). The answer is incomplete and missing key concepts.`;
    }
  ...
  useEffect(() => {
    if (stage === 'interview' && !isSpeaking) {
      const moodInterval = setInterval(() => {
        const moods = ['thinking', 'listening', 'neutral'];
  ...
  const handleStartInterview = () => {
    if (hasReachedLimit()) {
      setShowPaywall(true);
      speak("I'm sorry, but you've used all 3 of your free interviews for today. To continue practicing, please upgrade to premium!");
      return;
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50">
      <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        <AnimatePresence mode="wait">
          ...
          {/* Setup Stage - Role, Level, Duration Selection */}
          {stage === 'setup' && (
            <motion.div
              key="setup"
              initial={{ opacity: 0 }}
            ...
            >
              ...
              >
                <motion.button
                  whileHover={{ scale: config.role && config.level && config.duration ? 1.05 : 1 }}
                  whileTap={{ scale: config.role && config.level && config.duration ? 0.95 : 1 }}
                  onClick={handleStartInterview}
                  disabled={!config.role || !config.level || !config.duration}
                  className="px-12 py-5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-full font-bold text-lg shadow-xl hover:shadow-2xl transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-3 mx-auto"
                >
                  Start Interview
          ...
          {stage === 'intro' && (
            ...
            >
              ...
              >
                <div className="w-full h-full rounded-full overflow-hidden border-4 border-white relative bg-gradient-to-br from-indigo-200 to-purple-200 flex items-center justify-center">
                  <User className="w-24 h-24 text-indigo-600" strokeWidth={1.5} />
                  {isSpeaking && (
                    <motion.div
          ...
          {/* Interview Stage - This is where the magic happens! */}
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      <button
                        ...
                        className={`py-3 rounded-xl font-semibold transition-all ${
                          voiceEnabled && apiKeyConfigured ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-700'
                        } disabled:opacity-50 disabled:cursor-not-allowed`}
                        title={!apiKeyConfigured ? "API key required for voice" : voiceEnabled ? "Mute Alex" : "Unmute Alex"}
                      >
                        {voiceEnabled && apiKeyConfigured ? <Volume2 className="w-5 h-5 mx-auto" /> : <VolumeX className="w-5 h-5 mx-auto" />}
                      </button>
                      <button
                        onClick={repeatQuestion}
                        disabled={!currentQuestion || isSpeaking || !apiKeyConfigured}
                        className="py-3 rounded-xl font-semibold transition-all bg-indigo-100 text-indigo-700 hover:bg-indigo-200 disabled:opacity-50 disabled:cursor-not-allowed"
                        title={!apiKeyConfigured ? "API key required for voice" : "Repeat Question"}
                      >
                ...
                >
                  <div className="flex items-start justify-between mb-6">
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-2 bg-amber-50 text-amber-700 rounded-xl font-semibold hover:bg-amber-100 transition-colors"
                    >
                  ...
                  <div className="space-y-4">
                    <div className="relative">
                      <textarea
                        ...
                        onFocus={() => setInterviewerMood('listening')}
                        placeholder="Type your answer here... (or use voice input)"
                        className="w-full h-48 p-4 border-2 border-gray-200 rounded-2xl focus:border-indigo-500 focus:outline-none resize-none text-gray-900"
                        disabled={isGrading}
                      />
                      {isRecording && (
                        <motion.div
                    ...
                    <div className="flex items-center justify-between">
                      <button
                        ...
                        className="px-6 py-3 text-gray-600 hover:text-gray-900 font-semibold disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                      >
                        Previous Question
                      </button>

                      <motion.button
                        whileHover={{ scale: currentAnswer.trim() && !isGrading ? 1.05 : 1 }}
                        whileTap={{ scale: currentAnswer.trim() && !isGrading ? 0.95 : 1 }}
                        onClick={handleSubmitAnswer}
                        disabled={!currentAnswer.trim() || isGrading}
                        className="px-8 py-4 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-full font-bold shadow-lg hover:shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                      >
                        ...
                        >
                            ...
                            {answer.score && (
                              <div className="flex items-center gap-2">
                                <span className={`text-lg font-bold ${
                                  answer.score >= 85 ? 'text-emerald-600' :
                                  answer.score >= 70 ? 'text-orange-500' :
                                  'text-rose-600'
                                }`}>
                                  {answer.score}%
                                </span>
                          ...
                          {answer.strengths && answer.strengths.length > 0 && (
                            <div className="mt-2 flex flex-wrap gap-2">
                              {answer.strengths.slice(0, 2).map((strength, idx) => (
              ...
              <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-3xl mx-auto">
                ...
                <div className="space-y-4 mb-8">
                  ...
                  {answers.map((answer, i) => (
                    <div key={answer.questionId} className="border-2 border-gray-200 rounded-2xl p-6">
                      <div className="flex items-center justify-between mb-4">
                        <span className="font-bold text-gray-900">Question {i + 1}</span>
                        {answer.score && (
                          <div className={`px-4 py-1.5 rounded-full font-bold ${
                            answer.score >= 85 ? 'bg-emerald-100 text-emerald-700' :
                            answer.score >= 70 ? 'bg-orange-100 text-orange-700' :
